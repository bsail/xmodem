...
# Development
For developers looking to extend, bug fix, build, and test libXMODEM with dependencies and test infrastructure included in the source tree. This version differs from original version by [caseykelso](https://github.com/caseykelso/xmode): it uses Ceedling build system with Unity tests; it also has several changes in transmitter state machine in order to properly work with `rz` utility.
