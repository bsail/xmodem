...
enum XMODEM_TRANSMIT_STATES {XMODEM_TRANSMIT_INITIAL, XMODEM_TRANSMIT_WAIT_FOR_NACK, XMODEM_TRANSMIT_SEND_REQUEST_FOR_TRANSFER, XMODEM_TRANSMIT_WAIT_FOR_TRANSFER_ACK, XMODEM_TRANSMIT_TIMEOUT_TRANSFER_ACK,
                    XMODEM_TRANSMIT_TIMEOUT_WAIT_READ_BLOCK, XMODEM_TRANSMIT_ABORT_TRANSFER, XMODEM_TRANSMIT_READ_BLOCK, XMODEM_TRANSMIT_TRANSFER_ACK_RECEIVED,
                    XMODEM_TRANSMIT_TRANSFER_COMPLETE, XMODEM_TRANSMIT_BLOCK_RECEIVED, XMODEM_TRANSMIT_INVALID_BLOCK, XMODEM_TRANSMIT_ACK_BLOCK, XMODEM_TRANSMIT_VALID_BLOCK,
                    XMODEM_TRANSMIT_UNKNOWN } typedef xmodem_transmit_state_t;
...
bool xmodem_init();
bool xmodem_process_transmit(const uint32_t current_time);
bool xmodem_process_receive(const uint32_t current_time);
bool xmodem_cleanup();
